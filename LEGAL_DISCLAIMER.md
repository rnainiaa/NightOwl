# ⚠️ LEGAL DISCLAIMER - NIGHTOWL FRAMEWORK

## USAGE DISCLAIMER

**NIGHTOWL IS A PENETRATION TESTING FRAMEWORK INTENDED SOLELY FOR LEGITIMATE PURPOSES WITHIN AN AUTHORIZED FRAMEWORK.**

## 🚨 MANDATORY TERMS OF USE

### 1. WRITTEN AUTHORIZATION
The use of NightOwl **IMPERATIVELY** requires:
- A signed penetration testing contract
- Written authorization from the system owner
- A clearly defined testing scope
- Specified time windows

### 2. LEGAL FRAMEWORK
🔵 Compliance with the Criminal Code of Canada:
All activities must be conducted in full compliance with the Criminal Code of Canada, particularly the provisions relating to computer-related offences, including:
- Section 342.1 – Unauthorized use of a computer
- Section 430(1.1) – Mischief in relation to computer data
- Section 402.2 – Identity fraud
- Section 403 – Identity theft
- Section 380 – Fraud
Any technical intervention (penetration testing, vulnerability assessment, digital forensics investigations, etc.) must be carried out only with prior written authorization from the lawful owner of the systems.

🔵 Protection of Personal Information
Activities involving the processing of data must comply with Canadian privacy legislation, including:
- Personal Information Protection and Electronic Documents Act (PIPEDA) – applicable to the private sector at the federal level
- Applicable provincial privacy laws where relevant (e.g., Quebec’s Law 25)

The following fundamental principles must be respected:

- Informed consent
- Limitation of collection
- Use restricted to authorized purposes
- Secure data retention
- Right of access and correction

🔵 Respect for Privacy
Any collection, analysis, or retention of personal information must be:
- Strictly necessary for the authorized mandate
- Proportionate to the intended objective
- Protected by appropriate technical and organizational safeguards
No interception, monitoring, or access to private communications may be conducted without proper legal authority or a judicial warrant where required.

### 3. USAGE RESTRICTIONS

**ABSOLUTE PROHIBITIONS:**
- ❌ Unauthorized production environments
- ❌ Third-party systems without consent
- ❌ Exfiltration of personal data
- ❌ Destructive data modification
- ❌ Denial of Service (DoS/DDoS)
- ❌ Illicit lucrative activities

## 📋 COMPLIANCE CHECKLIST

Before any use, verify:

### ✅ Authorizations
- [ ] Signed pentest contract
- [ ] Scope defined in writing
- [ ] Emergency contacts established
- [ ] Civil liability insurance

### ✅ Technical
- [ ] Isolated test environment
- [ ] Backups performed
- [ ] Monitoring disabled on targets
- [ ] Non-liability clauses signed

### ✅ Legal
- [ ] Compliance with client security policy
- [ ] Adherence to agreed operation hours
- [ ] Destruction of implants after testing
- [ ] Full activity report

## 📞 LEGAL CONTACTS

In case of doubt about the legality of an action:

1. **STOP IMMEDIATELY** the operation
2. Contact the project's legal manager
3. Consult the legal department
4. Document the situation

## 🔐 BEST PRACTICES

### Logging
- Timestamped and cryptographically signed logs
- Retention of authorization proofs
- Complete documentation of actions

### Ethics
- Principle of proportionality
- Minimization of impacts
- Transparency with the client
- Post-test data destruction

## 📝 MANDATORY DOCUMENTATION

For each operation, maintain:

1. Signed **Service Contract**
2. Detailed **Test Scope**
3. **Roadmap** of actions
4. **Complete Logs** of activity
5. **Final Report** with proofs

## 🏛️ REGULATORY FRAMEWORK

### International
- Computer Fraud and Abuse Act (USA)
- UK Computer Misuse Act 1990
- Budapest Convention on Cybercrime
- GDPR: General Data Protection Regulation

## ⚠️ LIABILITY

The NightOwl user assumes full responsibility:
- For the legality of their actions
- For compliance with the contractual framework
- For the consequences of their operations
- For the protection of processed data

**IN CASE OF ILLEGAL USE, YOU ASSUME ALL CRIMINAL AND CIVIL LIABILITIES.**

---

**LAST UPDATE: 2026-01-01**
**VERSION: 1.0**
**LEGAL STATUS: AUTHORIZED TESTING TOOL ONLY**
